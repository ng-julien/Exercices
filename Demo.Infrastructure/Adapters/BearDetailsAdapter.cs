namespace Demo.Infrastructure.Adapters
{
    ...
    internal class BearDetailsAdapter : AnimalAdapter<BearDetails, NotFoundBearDetails>, IBearDetailsAdapter
    {
        ...
        {
            var foods = createBear.Foods.Select(foodId => new AnimalEat { FoodId = foodId }).ToList();

            var bear = new Animal
                           {
                               FamilyId = FamilyCode.Bear, AnimalEats = foods, Name = createBear.Name, Legs = createBear.Legs
                           };
